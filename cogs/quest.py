...
class Quest(BaseCog):
    ...
    async def cog_load(self):
        ...
        if not self.settings.enabled or not self.bot.danger_settings_dict["allow_auto_quest"]:
            if not self.bot.danger_settings_dict["allow_auto_quest"]:
                await self.bot.log("Current Auto quests is in a experimental stage. Inorder to avoid issues, we have locked auto quest until user manually alows it from `owo-dusk/config/danger.toml` file. Check the file for more details!", "#e6e65a") 
            try:
